...
//根据首字母对数组进行排序
Pinyin.paixu = function(params) {
    //根据首字母对专辑进行排序
    let szmMap = {}
    for(let param of params){
        let szm = Pinyin.GetJP(param).substring(0,1);
        if(!szmMap[szm]){
            szmMap[szm] = []
        }
        szmMap[szm].push(param);
    }
    let keys = Object.keys(szmMap).sort();
    params = []
    for(let key of keys){
        params = params.concat(szmMap[key].sort());
    }
    return params
}
